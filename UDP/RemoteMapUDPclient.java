...
public class RemoteMapUDPclient {
	...
	private String get(String key, String server_address, int server_port){
		...
		try {
			...
			String data_send = key;
			DatagramPacket packet_send = new DatagramPacket(data_send.getBytes(), data_send.length(),serverAddress,server_port);
			socket.send(packet_send);
			
			//Respuesta del servidor
			
			byte[] msg_send = new byte[256];//Buffer
			
			DatagramPacket packet_server = new DatagramPacket(msg_send, msg_send.length);
			//recibimos respuesta servidor
			socket.receive(packet_server);
			
			data_send = new String(packet_server.getData()).trim();
			resposta = data_send;
